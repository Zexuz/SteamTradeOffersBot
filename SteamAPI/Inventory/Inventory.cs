...
namespace SteamAPI.Inventory
{
    public class Inventory
    {
        ...
        protected Inventory(Item[] items, int resultSuccess)
        {
            IsGood = resultSuccess == 1;
            Items = items;
        }

        public static Inventory GetInventory(ulong steamId, SteamWeb steamWeb, int appId)
        {
            try
            {
                ...
                return new Inventory(items, response.Success);
            }
            catch (Exception e)
            {
